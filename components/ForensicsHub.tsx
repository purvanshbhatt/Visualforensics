...
const ForensicsHub: React.FC<ForensicsHubProps> = ({ jumpToTimeline, findCaseTerm }) => {
    ...
    return (
        <div className="space-y-12">
            {/* Timeline Section */}
            <div id="timeline">
                <h2 className="text-3xl font-bold text-center mb-8 text-cyan-400">Digital Forensics Timeline</h2>
                <div className="relative max-w-2xl mx-auto border-l-2 border-cyan-700/50 pl-8 space-y-8">
                    {timelineSteps.map((step, index) => (
                        ...
                        <div key={step.id} ref={el => { timelineRefs.current[index] = el; }} className="relative">
                            <div className="absolute -left-[42px] top-1.5 h-6 w-6 bg-gray-900 border-2 border-cyan-500 rounded-full"></div>
                            <button
                                onClick={() => setExpandedStep(expandedStep === step.id ? null : step.id)}
                                className="w-full text-left flex justify-between items-center p-4 bg-gray-800 rounded-lg hover:bg-gray-700 transition-colors"
                            >
                            ...
                            {expandedStep === step.id && (
                                <div className="mt-2 p-4 bg-gray-800/70 border-l-2 border-cyan-600 rounded-r-lg space-y-3 animate-fade-in-down">
            ...
            <div id="case-library" ref={caseLibraryRef}>
                <h2 className="text-3xl font-bold text-center mb-8 text-cyan-400">Case Library</h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gray-800/50 rounded-lg">
                        <div className="md:col-span-2 relative">
                            <input
                                ...
                                className="w-full p-2 pl-10 bg-gray-700 border border-gray-600 rounded-md focus:ring-2 focus:ring-cyan-500 focus:outline-none"
                            />
                        ...
                        >
                            {incidentTypes.map(type => <option key={type} value={type}>Type: {type}</option>)}
                        </select>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredCases.map(c => (
                            <div key={c.id} onClick={() => setSelectedCase(c)} className="bg-gray-800 p-4 rounded-lg border border-gray-700 hover:border-cyan-500 cursor-pointer transition-all space-y-2 transform hover:-translate-y-1">
